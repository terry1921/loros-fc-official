...
const CustomNavLink = ({ to, label, closeMenu }: { to: string, label: string, closeMenu: () => void }) => (
  <NavLink
    to={to}
    onClick={closeMenu}
    className={({ isActive }) =>
      `text-sm font-bold uppercase tracking-wider hover:text-yellow-400 transition-colors ${isActive ? 'text-yellow-400' : 'text-white'}`
    }
  >
    {label}
  </NavLink>
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-emerald-900 shadow-lg py-2' : 'bg-transparent py-6'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        ...
        <div className="hidden md:flex gap-8 items-center">
          <CustomNavLink to="/" label="Inicio" closeMenu={closeMenu} />
          <CustomNavLink to="/squad" label="Equipo" closeMenu={closeMenu} />
          <CustomNavLink to="/news" label="Noticias" closeMenu={closeMenu} />
          <CustomNavLink to="/shop" label="Tienda" closeMenu={closeMenu} />
