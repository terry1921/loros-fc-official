...
export const LastMatchForm: React.FC = () => {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow relative">
      ...
      <h3 className="text-xl font-bold mb-4">Editar último partido</h3>
      ...
      {lastMatch ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Rival</label>
            <input type="text" name="opponent" value={lastMatch.opponent} onChange={handleLastMatchChange}
                   className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Logotipo del rival</label>
            <input type="text" name="opponentLogo" value={lastMatch.opponentLogo}
                   onChange={handleLastMatchChange}
                   className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm"/>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">Marcador</label>
          ...
          <div className="flex items-center">
            ...
            <label htmlFor="home" className="ml-2 block text-sm text-gray-900">Partido de local</label>
          </div>
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700">Goleadores</label>
            {(lastMatch.scorers || []).map((scorer, index) => (
              <div key={index} className="flex items-center gap-2 mt-2">
                ...
                <input
                  ...
                  className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 sm:text-sm"
                />
                <button onClick={() => removeScorer(index)} className="text-red-500">Eliminar</button>
              </div>
            ))}
            <button onClick={addScorer} className="mt-2 text-emerald-600">Agregar goleador</button>
          </div>
          <div className="md:col-span-2">
            <button onClick={async () => {
              ...
              try {
                ...
                setIsSubmitting(false);
                    ...
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-lg">Guardar
              último partido
            </button>
          </div>
        </div>
      ) : <p>No hay información del partido para mostrar.</p>}
    </div>)
